        ...
        if (validate(s)) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (z instanceof ConditionalSubscriber) {
                    parents[i] = new FlowableOnAssembly.OnAssemblyConditionalSubscriber<T>((ConditionalSubscriber<? super T>)z, assembled);
                } else {
                    parents[i] = new FlowableOnAssembly.OnAssemblySubscriber<T>(z, assembled);
                }
            }
